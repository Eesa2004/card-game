...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print("Number of cards remaining:", self.__Deck.GetNumberOfCards())
    ...
    def __SetupGame(self):
        ...
        if Choice == "L":
            selectFile = input("Enter a file name to load")
            if not self.__LoadGame(selectFile):
...
##                Choice = input("(enter 1-5 to specify position of key) or (D)iscard five cards from the deck:> ")
                ...
                for i in range(0,self.__Hand.GetNumberOfCards()):
                    if self.__Hand.GetCardDescriptionAt(i)[0] == "K" or self.__Hand.GetCardDescriptionAt(i)[0] == "W":
                        keyFound = True
                if keyFound:
                    Choice = input("(enter 1-4 to specify position of key):> ")
                else:
                    input(f"\nNo key in hand so must discard five cards from the deck instead.\nPlease press (Enter) to continue:> ")
                    Choice = "D"
                print()
            ...
            elif self.__Deck.GetCardDescriptionAt(0) == "Retr":
                CurrentCard = self.__Deck.RemoveCard(self.__Deck.GetCardNumberAt(0))
                print()
                ...
                print()
                print(self.__Discard.GetCardDisplay())
                Choice = input("please enter Card you would like to take from discard pile:> ")
                CurrentCard.getDiscard(self.__Discard, self.__Hand, self.__Discard.GetCardNumberAt(int(Choice)-1), CardChoice)
                self.__Discard.AddCard(CurrentCard)
...
#                     ChoiceAsInteger -= 1
                ...
                if Hand.GetCardDescriptionAt(ChoiceAsInteger)[0] == "K":
                    ...
                    return
                elif Hand.GetCardDescriptionAt(ChoiceAsInteger)[0] == "W":
                    CardToMove = Hand.RemoveCard(Hand.GetCardNumberAt(ChoiceAsInteger))
                    Discard.AddCard(CardToMove)
                    return
        Count = 0
...
class RetrieveCard(Card):
    ...
    def getDiscard(self, Discard, Hand, Choice, CardChoice):
        CardToMove = Discard.RemoveCard(Choice)
